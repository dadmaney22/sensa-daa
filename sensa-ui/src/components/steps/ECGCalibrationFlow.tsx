...
import ecgPlacementSvg from '../../assets/ecg.svg'; 
import ecgDevicePng from '../../assets/eegHub.png';

export default function ECGCalibrationFlow({ onFinish }: { onFinish: () => void }) {
  ...
  // Live hub readout for Step 2 (which channel ECG is on, sensor detected, etc.)
  const hubConnected = !!pluxStatus?.running && !!pluxStatus?.device_address;
  const ecgPort = pluxStatus?.channel_map?.ecg;
  const ecgSensor = pluxStatus?.sensors?.find(s => s.type === 'ecg');

  // Step 2: poll the hub so the setup screen shows, live, which channel the
  // ECG sensor is on and whether the hub actually reports a sensor there.
  useEffect(() => {
    if (step !== 2) return;
    let cancelled = false;
  ...
  // reach the Signal Check step; reads this flow's ecg_raw channel.
  useEffect(() => {
    if (step < 3) return;
    ...
    ws.onmessage = (event) => {
      ...
      if (data.ecg_raw === undefined || data.ecg_raw === null) return;
      setWsState('streaming');
      setChartData(prev => {
        const newData = [...prev, { uv: data.ecg_raw }];
  ...
  // Real 30s baseline recording driven by the backend PLUX recorder.
  const startRealRecording = async () => {
    ...
    try {
      ...
      setTimeout(async () => {
        ...
        // Backend assesses stability over the full recording; read the ECG verdict.
        const q = stopInfo?.quality?.ecg;
    ...
    <div className="animate-in fade-in slide-in-from-right-4 duration-300">
      ...
      {step === 4 && recordingState === 'done' && baseline?.stable !== false && (
        <div className="mb-6 flex items-center gap-2 rounded-lg border border-green-200 bg-green-50 p-4 text-sm font-medium text-green-800">
          ...
          ECG Sensor Successfully calibrated.
        </div>
      )}
      ...
      <div className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm md:p-8">
        ...
        {step === 1 && (
          <div className="grid grid-cols-1 gap-8 md:grid-cols-2">
            ...
            <div className="flex flex-col items-center justify-center rounded-lg border border-gray-200 bg-white py-6">
              {/* Try swapping ecgPlacementSvg with your SVG */}
              <img src={ecgPlacementSvg} alt="ECG Placement Guide" className="w-full max-w-sm object-contain" />
              <div className="mt-4 text-xs text-gray-400">If image breaks, check the filename in ECGCalibrationFlow.tsx</div>
            </div>
              ...
              <div>
                ...
                <div className="space-y-2 px-1">
                  ...
                  <label className="flex cursor-pointer items-center gap-3 text-sm text-gray-700 hover:text-gray-900">
                    ...
                    Ensure skin is dry before attaching electrodes
                  </label>
                </div>
              </div>
          ...
          <div className="grid grid-cols-1 gap-8 md:grid-cols-2">
            <div className="space-y-4">
               <div className="rounded-lg border border-blue-200 bg-blue-50 p-4 text-xs leading-relaxed text-blue-800">
                 The biosignalsplux ECG sensor is compatible with all analog input channels of the 4-channel and 8-channel biosignalsplux hub, but incompatible with the reference/ground and digital I/O ports. Connect the sensor to an analog input to use it with this device.
               </div>
               
               <div className="flex items-center justify-center rounded-lg border border-gray-200 bg-white py-6">
                 <img src={ecgDevicePng} alt="Device hub connection guide" className="w-full max-w-sm object-contain" />
               </div>
               ...
               <div className="rounded-lg border border-gray-200 bg-white">
                 ...
                 <div className="space-y-2 p-4 text-sm">
                   ...
                   <div className="flex justify-between">
                     <span className="text-gray-600">ECG channel</span>
                     <span className="font-medium text-gray-900">{ecgPort !== undefined ? ecgPort : '—'}</span>
                   </div>
                   <div className="flex justify-between">
                     <span className="text-gray-600">Sensor on channel</span>
                     <span className={`font-medium ${ecgSensor?.detected ? 'text-green-600' : 'text-gray-500'}`}>
                       {ecgSensor?.detected
                         ? `Detected${ecgSensor.clas != null ? ` (class ${ecgSensor.clas})` : ''}`
              ...
              <div>
                ...
                <div className="space-y-2 px-1">
                  ...
                  <label className="flex cursor-pointer items-center gap-3 text-sm text-gray-700 hover:text-gray-900">
                    ...
                    <span className={`ml-auto rounded px-2 py-0.5 text-[11px] font-medium ${hubConnected && ecgPort !== undefined ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'}`}>
                      {hubConnected && ecgPort !== undefined ? `Channel ${ecgPort}` : 'Detecting…'}
                    </span>
                  </label>
                  <label className="flex cursor-pointer items-center gap-3 text-sm text-gray-700 hover:text-gray-900">
                    ...
                    <span className={`ml-auto rounded px-2 py-0.5 text-[11px] font-medium ${
                      ecgSensor?.detected ? 'bg-green-100 text-green-700'
                      : hubConnected ? 'bg-yellow-100 text-yellow-700'
                      : 'bg-gray-100 text-gray-500'}`}>
                      {ecgSensor?.detected
                        ? `ECG sensor${ecgSensor.clas != null ? ` (class ${ecgSensor.clas})` : ''}`
                        : hubConnected ? `Default ch ${ecgPort ?? '?'}`
                        : 'Detecting…'}
                    </span>
                  </label>
                </div>
        ...
        {step === 3 && (
          <div className="grid grid-cols-1 gap-8 md:grid-cols-2">
            <div>
              <p className="mb-4 text-center text-sm font-semibold text-gray-700">Live ECG waveform preview:</p>
                  ...
                  <ResponsiveContainer width="100%" height="100%">
                    <LineChart data={chartData} margin={{ top: 4, right: 8, left: 4, bottom: 20 }}>
                      ...
                      <YAxis
                        ...
                        tick={{ fontSize: 10, fill: '#9ca3af' }}
                        width={48}
                        label={{ value: 'ECG (raw)', angle: -90, position: 'insideLeft', offset: 10, fontSize: 10, fill: '#9ca3af' }}
                      />
                      <Line
                        type="monotone"
                        dataKey="uv"
            ...
            <div className="space-y-6">
              ...
              <div className="rounded-lg border border-gray-200 bg-white">
                 ...
                 <ul className="space-y-2 p-4 text-sm text-gray-700">
                   <li>1. Sit still and breathe normally</li>
                   <li>2. Avoid muscle movement and talking</li>
                   <li>3. Keep arms relaxed</li>
                 </ul>
              </div>
                  ...
                  <ResponsiveContainer width="100%" height="100%">
                    <LineChart data={chartData} margin={{ top: 2, right: 4, left: 4, bottom: 2 }}>
                      <YAxis
                        ...
                        width={42}
                        label={{ value: 'ECG', angle: -90, position: 'insideLeft', offset: 10, fontSize: 9, fill: '#9ca3af' }}
                      />
